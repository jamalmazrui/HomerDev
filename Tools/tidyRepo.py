...
Run it from the project folder, for example C:\\EdSharp, with no arguments to see what it would do:
...
def readVersionMessage():
    ...
    try:
        ...
        if match:
            return f"{appName()} {match.group(1)}"
    except OSError:
        pass
    return f"Update {appName()}"


def appName():
    """The app name, taken from the folder this repository sits in.

    Nothing in this script is hardcoded to one program: C:\\EdSharp yields
    "EdSharp", C:\\JobDo yields "JobDo". Everything a message needs to name
    comes from here.
    """
    return os.path.basename(os.path.normpath(pathRoot)) or "this project"
...
def main():
    parser = argparse.ArgumentParser(description="Tidy this Homer Tools repository.")
    ...
    if not os.path.isdir(os.path.join(pathRoot, ".git")):
        say("This is not a git repository. Run tidyRepo.py from the project folder, for example C:\\EdSharp.")
        return 1
    ...
    if bNeedRewrite:
        sBackup = os.path.join(os.path.dirname(pathRoot),
                               f"{appName()}_backup_{datetime.datetime.now():%Y%m%d_%H%M%S}")
    ...
    if lStray:
        ...
        # the file being searched for the name. build/<App>.nvda-addon
        ...
        if lAdd:
            with open(pathIgnore, "a", encoding="utf-8", newline="\n") as fileIgnore:
                ...
                # <App>.jss written for the stale root copy also
                # matches jaws/<App>.jss, which is the live script
